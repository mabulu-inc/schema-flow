  ...
  return await withClient(
    ...
    async (client) => {
      ...
      try {
        ...
        if (parsedFunctions.length > 0) planOptions.functions = parsedFunctions;
        ...
        try {
          ...
          if (plan.structureOps.length > 0 || parsedFunctions.length > 0) {
            ...
            const isPostFunctionOp = (o: { type: string; sql?: string }) =>
              ...
              o.type === "grant_function" ||
              o.type === "revoke_function" ||
              (o.type === "set_comment" &&
                o.sql != null &&
...
export async function runValidate(config: SchemaFlowConfig): Promise<ValidationResult> {
  ...
  return await withClient(config.connectionString, async (client) => {
    ...
    const plan = await buildPlan(client, expandedSchemas, config.pgSchema, {
      ...
      functions: parsedFunctions.length > 0 ? parsedFunctions : undefined,
    });
