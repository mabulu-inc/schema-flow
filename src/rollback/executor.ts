...
import { computeRollback, type RollbackResult } from "./index.js";
...
export async function runDown(
  ...
  return await withClient(
    ...
    async (client) => {
      ...
      logger.info(
        `Last run: ${lastRun.run_id} (${lastRun.status}, ${lastRun.ops_executed} ops, ${lastRun.started_at})`,
      );
      ...
      if (rollbackPlan.hasDestructive && !options.allowDestructive) {
        ...
        return {
          success: false,
          operationsExecuted: 0,
          errors: ["Destructive operations blocked"],
          plan: rollbackPlan,
        };
      }
